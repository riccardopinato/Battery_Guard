...
class BatteryRing extends StatelessWidget {
  ...
  Color _ringColor(BuildContext context) {
    ...
    if (temperatureC >= 45) return scheme.error;
    if (temperatureC >= 40) return const Color(0xFFF59E0B);
    if (level <= 15) return scheme.error;
    if (isCharging) return const Color(0xFF2F80ED);
    return scheme.primary;
  }

  @override
  Widget build(BuildContext context) {
    final color = _ringColor(context);
    return TweenAnimationBuilder<double>(
      tween: Tween(begin: 0, end: level / 100),
